...
UnstructuredImmersedEllipseProblem()
{
  //Change maximum residue to give a bit of room for solution
   ...
   for (unsigned inod=0;inod<num_nod;inod++)
    {
     ...
     //Pin x-velocity everywhere except outlet (2) boundaries
     //of the external rectangular box
     if((ibound!=2)) {nod_pt->pin(0);}
...
set_boundary_velocity()
{
  ...
  for (unsigned inod=0;inod < num_nod; inod++) 
    { 
      double y= this->Fluid_mesh_pt->boundary_node_pt(ibound,inod)->x(1);
      if (y > y_max)
	{
	  y_max=y;
	}
 ...
 for(unsigned ibound=0;ibound<4;ibound++)
  {
   ...
   //If we are on the  outlet only zero the 
